...
// Defaults used when CMS data is not available
...
export default function StoreHome() {
  const { products, loading: productsLoading, isNewProduct } = useFeaturedProducts(4);
  const { getSection, loading: cmsLoading } = useHomepageSections();

  const hero = getSection("hero");
  const featureBar = getSection("feature_bar");
  const saleBanner = getSection("sale_banner");
  const newsletter = getSection("newsletter");

  const features = featureBar?.content?.features || defaultFeatures;

  // Split hero title by highlight_text
  const heroTitle = hero?.title || "Elevate Your Style Game";
  const highlightText = hero?.content?.highlight_text || "Style Game";
  const titleParts = heroTitle.split(highlightText);

  return (
    <StoreLayout>
      {/* Hero Section */}
      {(cmsLoading || !hero || hero.is_enabled !== false) && (
        <section className="relative min-h-[70vh] flex items-center overflow-hidden">
          ...
          <div
            ...
            style={{ backgroundImage: `url('${hero?.image_url || "https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1920&h=1080&fit=crop"}')` }}
          />
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-2xl text-store-primary-foreground">
              <Badge className="mb-4 bg-store-accent text-store-accent-foreground px-4 py-1">
                {hero?.badge_text || "New Collection 2024"}
              </Badge>
              <h1 className="font-display text-4xl md:text-6xl font-bold mb-4 leading-tight">
                {titleParts[0]}
                <span className="block text-store-accent">{highlightText}</span>
                {titleParts[1]}
              </h1>
              <p className="text-lg md:text-xl mb-8 text-store-primary-foreground/90">
                {hero?.subtitle || "Discover premium fashion that speaks to your unique personality."}
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                <Button
                  ...
                  asChild
                >
                  <Link to={hero?.content?.cta_link || "/products"}>
                    {hero?.content?.cta_text || "Shop Now"} <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>
                {(hero?.content?.secondary_cta_text) && (
                  <Button
                    size="lg"
                    variant="outline"
                    ...
                    asChild
                  >
                    <Link to={hero?.content?.secondary_cta_link || "/products?filter=new"}>
                      {hero?.content?.secondary_cta_text}
                    </Link>
                  </Button>
                )}
                ...
                return (
                  <div key={feature.title} className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-store-muted flex items-center justify-center">
                      <FeatureIcon className="h-6 w-6 text-store-primary" />
                    </div>
                    <div>
      ...
      {(saleBanner?.is_enabled !== false) && (
        <section className="py-16 bg-gradient-to-r from-store-secondary via-store-primary to-store-secondary text-store-primary-foreground">
          <div className="container mx-auto px-4 text-center">
            {(saleBanner?.badge_text) && (
              <Badge className="mb-4 bg-store-accent text-store-accent-foreground">
                {saleBanner.badge_text}
              </Badge>
            )}
            <h2 className="font-display text-4xl md:text-5xl font-bold mb-4">
              {saleBanner?.title || "Up to 50% Off"}
            </h2>
            <p className="text-xl mb-8 opacity-90">
              {saleBanner?.subtitle || "Don't miss out on our biggest sale of the season!"}
            </p>
            <Button
              size="lg"
              ...
              asChild
            >
              <Link to={saleBanner?.content?.cta_link || "/products?filter=sale"}>
                {saleBanner?.content?.cta_text || "Shop Sale"}
              </Link>
            </Button>
          </div>
        </section>
          ...
          <div className="container mx-auto px-4 text-center max-w-xl">
            ...
            <div className="flex gap-2 max-w-md mx-auto">
              ...
              <Button className="bg-primary text-primary-foreground">
                {newsletter?.content?.button_text || "Subscribe"}
              </Button>
            </div>
          </div>
        </section>
