...
export default function ProductDetail() {
  ...
  const displayComparePrice = activeVariant?.compare_at_price ?? product?.compare_at_price ?? null;
  const displayQuantity = activeVariant ? (activeVariant.quantity || 0) : (product?.quantity ?? 0);
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              {discount > 0 && (
                <Badge className="bg-store-accent text-store-accent-foreground">
                  Save ৳{((displayComparePrice || 0) - displayPrice).toLocaleString()}
                </Badge>
              )}
            </div>
                  ...
                  {variants.filter(v => v.is_active).map((variant) => (
                    <Button
                      ...
                      className={cn(
                        ...
                        (variant.quantity || 0) === 0 && "opacity-50"
                      )}
                    ...
                    >
                      ...
                      {variant.price != null && (
                        <span className="ml-1 text-xs opacity-75">৳{variant.price.toLocaleString()}</span>
                      )}
                    </Button>
                  ))}
                </div>
