...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Generate Storage RLS policies
    const storagePolicies = policies.filter(p => false) // storage policies come from pg_policies on storage.objects
    // Query storage policies separately
    const { data: storagePoliciesRaw } = await supabaseAdmin.rpc('get_rls_policies')
    // We need a dedicated query for storage policies - use the existing ones from the migration
    sqlParts.push(`-- Note: Storage RLS policies should be recreated from your migration files.`)
    sqlParts.push(`-- Check supabase/migrations/ for storage.objects policies.\n`)
